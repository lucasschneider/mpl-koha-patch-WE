    ...
    function GetMessage(messageName, substitutions) {
        ...
        if (custom && custom.message.length > 0) {
            ...
            do {
                ...
                if (m) {
                    trans.push(m[1]);
                    if (custom.placeholders && custom.placeholders[m[2]] && custom.placeholders[m[2]].content && substs[custom.placeholders[m[2]].content]) trans.push(substs[custom.placeholders[m[2]].content]);else trans.push("$" + m[2] + "$");
                    str = m[3];
                }
            } while (m);
            trans.push(str);
