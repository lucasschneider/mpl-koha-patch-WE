...
function queryPSTAT(addr, city, queryB, secondPass) {
  notice = document.getElementById('tractNotice'), exceptionQuery = false, secondPass = secondPass, queryB = queryB;
...
// Add listener for Geocoder query response
browser.runtime.onMessage.addListener(function (message) {
  if (message && message.key === "receivedGeocoderQuery") {
    if (message.hasData) {
      ...
      } else if (!secondPass && !exceptionQuery) {
...
// Add listener for looking up the nearest library location
browser.runtime.onMessage.addListener(function (message) {
  if (message) {
    switch (message.key) {
...
browser.runtime.onMessage.addListener(function (request) {
  // If the geocoder query returned empty, check for MAD exceptions
  if (request.key === "requestMadExceptions") {
    browser.runtime.sendMessage({
      key: "getPstatByDist",
      addrVal: request.addrVal,
      lib: "Mad",
      tract: request.tract,
      zip: request.zip
    });
  } else if (request.key === "receivedMAD") {
    selectPSTAT(selectList, request.value, result, userEnteredAddress);
    // Set zip code
    if (queryB && zipEltB) {
