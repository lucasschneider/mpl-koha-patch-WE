...
if (/^https?\:\/\/scls-staff\.kohalibrary\.com\/cgi-bin\/koha\/circ\/waitingreserves\.pl.*/.test(location.href)) {
  var getExpiration = function getExpiration(date) {
    date.addDays(8);
  ...
  var WaitingHold = function WaitingHold(htmlTR) {
    var i = 0;
  ...
  var sortWaitingHolds = function sortWaitingHolds(waitingHolds, sortCode, dateType, limitDate) {
  ...
  var updateHoldTable = function updateHoldTable(waitingHolds) {
    var sortCodeVal = "",
        dateTypeVal = "",
        limitDateVal = "",
