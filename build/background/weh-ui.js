...
weh.ui = function () {
    ...
    browser.runtime.onConnect.addListener(function (port) {
        ...
        if (!m) return;
        if (m[1] != browser.runtime.id) return;
        var panelName = m[2];
        var panel = panels[panelName];

        if (!panel) panel = panels[panelName] = {
            name: panelName,
        ...
        port.onMessage.addListener(function (message) {
            ...
            switch (message.type) {
                ...
                case "weh#get-prefs":
                    var prefs = {};
                    if (typeof message.prefs == "undefined") prefs = weh.prefs.getAll();else {
                        if (!Array.isArray(message.prefs)) message.prefs = [message.prefs];
                ...
                case "weh#get-prefs-specs":
                    ...
                    if (typeof message.specs == "undefined") specs = allSpecs;else {
                        if (!Array.isArray(message.specs)) message.specs = [message.specs];
    ...
    return {
        ...
        update: function update(panelName, options) {
            var panel = panels[panelName] || {
                name: panelName,
                open: false,
                options: options
        ...
        open: function open(panelName, options) {
            options = options || {};
            var panel = panels[panelName] || this.update(panelName, options);
            UpdateOptions(panel.options);

            if (panel.options.type == "panel") browser.windows.getCurrent(function (currentWindow) {
                var width = 500;
                var cwcParam = {
                ...
                if (weh.is("chrome", "opera")) cwcParam.focused = true;
                if (weh.is("edge", "opera")) cwcParam.type = "popup";
                ...
                browser.windows.create(cwcParam, function (window) {
                    ...
                    function OnFocusChanged(focusedWindowId) {
                        browser.windows.getCurrent(function (currentWindow) {
                            if (currentWindow.id != window.id) try {
                ...
                GotoTab(url, function (foundTab) {
                    if (!foundTab) browser.tabs.create({
                        url: url
                    }, function (tabs) {});
                });
        ...
        close: function close(panelName) {
            var panel = panels[panelName];
            if (panel && panel.open) panel.post({
                type: "weh#close"
            });
        },
        ...
        post: function post() {
            ...
            if (typeof arguments[0] == "string") {
                ...
                message = arguments[1];
                var panel = panels[panelName];
                if (panel && panel.post) panel.post(message);
            } else {
                message = arguments[0];
                for (var panelName in panels) {
                    var panel = panels[panelName];
                    if (panel.post) panel.post(message);
                }
            }
        }
